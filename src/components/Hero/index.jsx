 ...
 */
const Hero = () => {
  const { hero, party } = useDB((db) => db);

  const firstName = party.girl.firstName;
  const lastName = party.girl.lastName;
  const title = `${firstName} ${lastName}`;
  const invitation = "Mis Quince";
      ...
      >
        ...
        >
          <Container>
            ...
            >
              {party.reception.date.split(" ").map((text, key) => (
                <TextMask key={key} variants={textVariants}>
                  {text}
                </TextMask>
              ))}
